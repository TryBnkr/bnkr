...
func (m *Repository) destDB(g *dal.Migration, c MigrationCommon) (string, error) {
	...
	case "ssh":
		...
		output, err := utils.RunSshCommand(conn, "gunzip < /"+c.MigrationName+" | mysql --max_allowed_packet=4096M -h "+g.DestDbHost+" -u "+g.DestDbUser+" -p"+g.DestDbPassword+" "+g.DestDbName)
...
` + output3
		...
		args = []string{"exec", helperPodName, "--kubeconfig", kubeconfigPath, "--", "sh", "-c", "gunzip < /" + c.MigrationName + " | " + "mysql --max_allowed_packet=4096M -h " + g.DestDbHost + " -u " + g.DestDbUser + " -p" + g.DestDbPassword + " " + g.DestDbName}
		cmd = exec.Command("kubectl", args...)
